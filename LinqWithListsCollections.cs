...
namespace LinqWithLists
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            um.AllStudentsFromMichiganState(1);
            ...
            Console.Write("Enter 1 for MSU student list or 2 for UM student list: ");

            try
            {
            int userInput = int.Parse(Console.ReadLine());
            um.UserInputUniversityIdStudents(userInput);
            }
            catch (Exception)
            {
    ...
    class UniversityManager
    {
        ...
        public UniversityManager()
        {
            ...
            students.Add(new Student { Id = 6, Name = "Neo", Gender = "male", Age = 23, UniversityId = 1 });
        }
        ...
        public void AllStudentsFromMichiganState(int Id)
        {
            IEnumerable<Student> msuStudents = from student in students
                                               join university in universities on student.UniversityId equals university.Id
                                               where university.Name == "Michigan State"
                                               select student;

            Console.WriteLine("Students from Michigan State University");
            foreach (Student student in msuStudents)
            {
                student.Print();
            }
        }

        public void UserInputUniversityIdStudents(int Id)
        {


            IEnumerable<Student> inputStudent = from student in students
                                                join university in universities on student.UniversityId equals university.Id
                                                where university.Id == Id
                                                select student;

            Console.WriteLine("Students from {0}", Id);
            foreach (Student student in inputStudent)
            {
                student.Print();
            }
        }

        //combining two List and put into a new list with Linq
